...
import { logout, puedeCapturar, canUpload, type Sesion } from '@/autenticacion/auth';
import { AREAS_FLOW, AREA_LABELS, AREA_COLORS } from '@/compartido/mock-data';
...
export default function Sidebar({ usuario, email, collapsed = false, onToggle }: Props) {
  ...
  const navItems = [
    ...
    // Registro maestro de clientes y specs: solo quienes gestionan órdenes.
    ...(canUpload(usuario?.rol)
...
function ClientesIcon({ active }: { active: boolean }) {
  ...
  return (
    <svg width="18" height="18" viewBox="0 0 18 18" fill="none" className="flex-shrink-0">
      <rect x="2" y="5" width="14" height="10" rx="1.5" stroke={c} strokeWidth="1.5" />
      <path d="M6 5V3.5A1.5 1.5 0 017.5 2h3A1.5 1.5 0 0112 3.5V5M2 9h14" stroke={c} strokeWidth="1.5" strokeLinecap="round" />
    </svg>
  );
}
