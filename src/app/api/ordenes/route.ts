...
import { rowToOrden, ORDEN_COLS, type OrdenRow } from '@/lib/orden-map';
import { generarAvance } from '@/lib/produccion';
...
export async function POST(req: Request) {
  ...
  const { rows: urows } = await query<{ rol: string }>('SELECT rol FROM usuarios WHERE email = $1', [actorEmail]);
