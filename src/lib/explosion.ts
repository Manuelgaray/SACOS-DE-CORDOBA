...
import { plantillaCorte, OPCIONALES_CORTE } from './produccion';
import type { ElementoCorte, UnidadMedida, GrupoCorte } from './mock-data';
...
// Elementos opcionales (faldón, válvulas, cintas, cordeles…) para "agregar".
export function opcionesCorte(): ElementoCorte[] {
  return OPCIONALES_CORTE.map((it) => ({
    nombre: it.nombre,
    piezasPorSaco: it.mult,
    ancho: 0,
    largo: 0,
