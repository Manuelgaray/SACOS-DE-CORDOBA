...
import { AREAS_FLOW, type Area, type Orden, type GrupoCorte } from './mock-data';
...
export const OPCIONALES_CORTE: PlantillaItem[] = [
  ...
  { nombre: 'Baffles', mult: 4, grupo: 'tela' },
  { nombre: 'Cinturones', mult: 4, grupo: 'cinturones' },
  { nombre: 'Cintas', mult: 1, grupo: 'cintas' },
  { nombre: 'Cordeles', mult: 1, grupo: 'cintas' },
];

function plantillaArea(area: Area, tipoSaco: string): PlantillaItem[] {
  if (area === 'corte') return plantillaCorte(tipoSaco);
...
export function generarAvance(orden: Orden): AvanceArea[] {
  ...
  return AREAS_FLOW.map((area) => {
    ...
    const items = plantillaArea(area, orden.tipo_saco);
