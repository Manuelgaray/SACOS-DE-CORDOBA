...
export function TablaCorteEditable({
  ...
  const opciones = useMemo(() => opcionesCorte(), []);
          ...
          <tbody className="divide-y divide-[#F0F5F0]">
            {elementos.map((el, idx) => (
              <tr key={idx}>
                ...
                <td className="py-1.5 px-2">
                  <input type="number" min={0} className={`${inCls} font-mono`} value={el.piezasPorSaco} onChange={e => onActualizar(idx, 'piezasPorSaco', e.target.value)} />
                </td>
                <td className="py-1.5 px-2">
                  <input type="number" min={0} step="0.01" className={`${inCls} font-mono`} value={el.ancho} onChange={e => onActualizar(idx, 'ancho', e.target.value)} />
                </td>
                <td className="py-1.5 px-2">
                  <input type="number" min={0} step="0.01" className={`${inCls} font-mono`} value={el.largo} onChange={e => onActualizar(idx, 'largo', e.target.value)} />
                </td>
                <td className="py-1.5 px-2">
...
export default function ExplosionMateriales({ orden }: { orden: Orden }) {
  ...
  const { setCorteElementos } = useProduccion();
  ...
  async function guardar() {
    ...
    try {
      ...
      setMsg({ tipo: 'ok', texto: 'Explosión de materiales guardada.' });
    } catch {
      setMsg({ tipo: 'error', texto: 'No se pudo conectar con el servidor.' });
    } finally {
      setGuardando(false);
    ...
    <div className="space-y-4">
      ...
      {editor && (
        <>
          ...
          <TablaCorteEditable elementos={elementos} onActualizar={actualizar} onAgregar={agregar} onQuitar={quitar} />
  ...
  return (
    <div className="overflow-x-auto rounded-lg border border-[#E8EFE9]">
      <div className={`px-3 py-2 text-xs font-semibold border-b border-[#E8EFE9] ${esTela ? 'bg-brand-green-50' : 'bg-[#F6F8F1]'}`}>
        {g.etiqueta}
        {!esTela && <span className="font-normal text-[#8A9A8C]"> · no es tela, cuenta aparte</span>}
      </div>
      <table className="w-full min-w-[640px] text-sm border-collapse">
        <thead>
          <tr className="text-left text-[10px] uppercase tracking-wide text-[#8A9A8C] bg-white">
            ...
            <th className="py-2 px-3 font-medium text-right">Pieza (pulg)</th>
            <th className="py-2 px-3 font-medium text-right">Piezas totales</th>
            <th className="py-2 px-3 font-medium text-right">Lineal (pulg)</th>
            <th className="py-2 px-3 font-medium text-right">cm</th>
